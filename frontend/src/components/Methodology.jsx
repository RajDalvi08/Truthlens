  ...
  return (
    <div className="max-w-6xl mx-auto space-y-24 animate-in fade-in duration-1000 mesh-bg pb-32">
      ...
      >
        ...
        {/* ── Hero ── */}
        <motion.div variants={itemVariants} className="pt-8 sm:pt-16 pb-10 sm:pb-20 border-b border-[#fdf8f5]/10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8">
            <div>
              <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className="w-2 h-2 bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]" />
                <span className="text-[8px] sm:text-[9px] font-black text-[#8d7b68] uppercase tracking-[0.3em] sm:tracking-[0.4em] italic">
                  Neural Intelligence Framework — v2.1
                </span>
              </div>
              <h1 className="text-4xl sm:text-6xl md:text-8xl font-black text-[#fdf8f5] tracking-tighter uppercase italic leading-[0.9]">
                System<br />
                <span className="text-[#8d7b68]">Methodology</span>
              </h1>
              <p className="text-[#d6c2b8] mt-4 sm:mt-8 max-w-2xl font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-[10px] sm:text-[11px] italic leading-relaxed">
                A transparent look at how TruthLens ingests, classifies, and scores news articles across the global information spectrum.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 md:flex md:flex-col gap-2 sm:gap-3 text-left md:text-right shrink-0">
              {[
                { label: "Bias Accuracy", value: "91.4%" },
                { label: "Sentiment Accuracy", value: "88.7%" },
                { label: "Articles Processed", value: "1.2M+" },
              ].map((m) => (
                <div key={m.label} className="bg-[#261a14]/60 border border-[#fdf8f5]/10 px-4 sm:px-6 py-2.5 sm:py-3">
                  <p className="text-[8px] sm:text-[9px] font-black text-[#8d7b68] uppercase tracking-[0.2em] sm:tracking-[0.3em] italic">{m.label}</p>
                  <p className="text-xl sm:text-2xl font-black text-[#fdf8f5] italic tracking-tighter">{m.value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div variants={itemVariants}>
          <div className="flex items-center gap-4 sm:gap-6 mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-black text-[#fdf8f5] uppercase italic tracking-tighter">
            ...
            {pipeline.map((step, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Header row */}
                  <div className="p-4 sm:p-6 md:p-8 flex items-center gap-4 sm:gap-8">
                    <div
                      className="w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center shrink-0 border shadow-xl transition-all duration-300"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 sm:gap-4 mb-1">
                        <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.3em] sm:tracking-[0.4em] italic" style={{ color: step.color }}>
                          Step {step.id}
                        </span>
                        <span className="text-[7px] sm:text-[8px] font-black text-[#4d3c2e] uppercase tracking-widest italic">
                          {step.subtitle}
                        </span>
                      </div>
                      <h3 className="text-lg sm:text-2xl font-black text-[#fdf8f5] uppercase italic tracking-tighter leading-none truncate sm:whitespace-normal">
                        {step.title}
                      </h3>
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                      ...
                      <HiOutlineArrowRight
                        className="w-4 h-4 sm:w-5 sm:h-5 text-[#8d7b68] transition-transform duration-300"
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className="px-4 sm:px-8 pb-4 sm:pb-8 border-t border-[#fdf8f5]/5"
                    >
                      <p className="text-xs sm:text-sm text-[#d6c2b8] font-black uppercase tracking-tight leading-relaxed italic opacity-90 mt-4 sm:mt-6">
                        {step.desc}
                      </p>
                    </motion.div>
                  )}
            ...
            {techStack.map((tech) => {
              ...
              return (
                <motion.div
                  ...
                  className="saas-card p-5 sm:p-8 group hover:border-[#fdf8f5]/30 transition-all cursor-default bg-[#261a14]/60 border-[#fdf8f5]/10 rounded-2xl shadow-2xl relative overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-24 h-24 bg-[#fdf8f5]/5 blur-[50px] pointer-events-none" />
                  <div className="w-10 h-10 sm:w-12 sm:h-12 mb-4 sm:mb-6 bg-[#fdf8f5]/5 border border-[#fdf8f5]/10 flex items-center justify-center text-[#fdf8f5] group-hover:bg-[#fdf8f5] group-hover:text-[#1a0f0a] transition-all shadow-xl">
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <p className="text-[8px] sm:text-[9px] font-black text-[#8d7b68] uppercase tracking-[0.2em] sm:tracking-[0.3em] italic mb-1 sm:mb-2">{tech.label}</p>
                  <h4 className="text-lg sm:text-xl font-black text-[#fdf8f5] uppercase italic tracking-tighter">{tech.name}</h4>
                </motion.div>
              );
            })}
          </div>
        ...
        {/* ── Scoring Legend ── */}
        <motion.div variants={itemVariants} className="saas-card p-6 sm:p-8 md:p-12 bg-[#261a14]/60 border-[#fdf8f5]/10 rounded-2xl shadow-2xl relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-[#fdf8f5]/[0.02] to-transparent pointer-events-none" />
          <h2 className="text-2xl sm:text-3xl font-black text-[#fdf8f5] uppercase italic tracking-tighter mb-6 sm:mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
            {[
              ...
              <div key={entry.label} className="flex flex-col items-center text-center p-4 sm:p-6 bg-[#1a0f0a] border border-[#fdf8f5]/5 hover:border-[#fdf8f5]/20 transition-colors group">
                <div className="w-3 h-3 mb-2 sm:mb-4 shadow-2xl" style={{ background: entry.color, boxShadow: `0 0 12px ${entry.color}80` }} />
                <p className="text-sm sm:text-base font-black italic tracking-tighter" style={{ color: entry.color }}>{entry.label}</p>
                <p className="text-[8px] font-black text-[#4d3c2e] uppercase tracking-widest mt-1 sm:mt-2 tabular-nums">{entry.range}</p>
              </div>
            ))}
          </div>
        </motion.div>
